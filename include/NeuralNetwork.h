...
#define __NN_VERSION__ "VERSION: 2.4.1"\n
...
#undef NB_CA1_1
#define NB | 𝗡𝗢_𝗕𝗔𝗖𝗞𝗣𝗥𝗢𝗣 SUPPORT FOR:
#define NB_CA1_1 | > CUSTOM_AF1
#endif
#define CACT1 1
#define ACTIVATION_FUNCTION CUSTOM_AF1
#define CA1_1 | > CUSTOM_AF1
#endif
...
#define INFORMATION LOVE __NN_VERSION__ MSG0 MSG1 MSG2 MSG3 MSG4 MSG5 MSG6 MSG7 MSG8 MSG9 MSG10 MSG11 MSG12 MSG13 MSG14 MSG15 MSG16 \n\n 𝗨𝗦𝗜𝗡𝗚[ƒx] AL A1_1 A2 A3 A4 A5 A6 A7 A8 A9 A1_10 A1_11 A1_12 A1_13 A1_14 CSTA CA1_1 CA2 CA3 CA4 CA5 | ~ |\n\n NB A9 A1_10 A1_11 A1_12 A1_13 A1_14 NB_CA1_1 NB_CA2 NB_CA3 NB_CA4 NB_CA5
...
// LOSS VARIABLES | V Because #6
    ...
    void print();
    String save_to_blob();
    bool load_from_blob(String buff);
};
#if defined(REDUCE_RAM_STATIC_REFERENCE)
...
void NeuralNetwork::pdestract()
{
    ...
    if (isAllocdWithNew)
    { // Because of undefined behavior in some MCUs like ESP32-C3
        unsigned int i = 0;
...
// #if !defined(USE_PROGMEM)
            ...
            */
            ...
            { // -1 because we need final-outputs(below) to be managed by user.
                break;
            }
    ...
    {
#if !defined(REDUCE_RAM_DELETE_OUTPUTS)
        for (unsigned int i = 0; i < numberOflayers - 1; i++)
        { // -1 because we need final-outputs to be managed by user.
            delete[] layers[i].outputs;
        }
#endif
    }
...
String NeuralNetwork::save_to_blob()
{
    unsigned int totalNumOfWeights = 0;
    String blob = "        ";
    blob += String(numberOflayers + 1);
    for (unsigned int n = 0; n < numberOflayers; n++)
    {
#if defined(ACTIVATION__PER_LAYER)
        blob += String(ActFunctionPerLayer[n]);
#endif
        blob += String(layers[n]._numberOfInputs);
        blob += String(layers[n]._numberOfOutputs);
#if !defined(NO_BIAS) and !defined(MULTIPLE_BIASES_PER_LAYER)
        blob += String(CAST_TO_LLONG_IF_NOT_INT_QUANTIZATION(*layers[n].bias));
#endif
        for (unsigned int i = 0; i < layers[n]._numberOfOutputs; i++)
        {
#if defined(MULTIPLE_BIASES_PER_LAYER)
            blob += String(CAST_TO_LLONG_IF_NOT_INT_QUANTIZATION(layers[n].bias[i]));
#endif
            for (unsigned int j = 0; j < layers[n]._numberOfInputs; j++)
            {
#if defined(REDUCE_RAM_WEIGHTS_LVL2)
                blob += String(CAST_TO_LLONG_IF_NOT_INT_QUANTIZATION(weights[totalNumOfWeights])); // I would had used i_j but I have totalNumOfWeights so... :)
#else
                blob += String(CAST_TO_LLONG_IF_NOT_INT_QUANTIZATION(layers[n].weights[i][j]));
#endif
                totalNumOfWeights++;
            }
        }
    }
    blob += String(totalNumOfWeights);
    ;
    return blob;
}

bool NeuralNetwork::load_from_blob(String buff)
{
    return false;
}
...
void NeuralNetwork::Layer::FdF_Individual_iEEPROM(const DFLOAT &input, const unsigned int &j) // Not my proudest implementation, ngl... but it does the job for now
{
    ...
    if (j == 0)
    { // if it is the first input then create output array (for the output layer of this current layer)
#if defined(REDUCE_RAM_DELETE_OUTPUTS)
        outputs = new DFLOAT[_numberOfOutputs];
#endif
#if defined(ACTIVATION__PER_LAYER)
...
#endif
#if defined(ALL_ACTIVATION_FUNCTIONS) or defined(Softmax)
DFLOAT NeuralNetwork::Layer::SoftmaxSum(const DFLOAT &x)
{
    DFLOAT tmp = exp(x);
    me->sumOfSoftmax += tmp;
    return tmp;
}
void NeuralNetwork::Layer::Softmax()
{
    for (int i = 0; i < _numberOfOutputs; i++)
    {
        outputs[i] /= me->sumOfSoftmax;
    }
}
#endif
...
#if defined(REDUCE_RAM_WEIGHTS_LVL2)
            if (me->weights[me->i_j] > 0)
                Serial.print(F_MACRO(" ")); // dont even bothered to opt. here lol
...
void NeuralNetwork::Layer::print_PROGMEM()
{
    ...
    for (unsigned int i = 0; i < _numberOfOutputs; i++)
    {
        ...
        for (unsigned int j = 0; j < _numberOfInputs; j++)
        {
            ...
            Serial.print(F_MACRO(" W:"));
#if defined(REDUCE_RAM_WEIGHTS_LVL2)
            if (PGM_READ_IDFLOAT(&me->weights[me->i_j]) MULTIPLY_BY_INT_IF_QUANTIZATION > 0)
                Serial.print(F_MACRO(" ")); // if gratter than 10 too or something would be nice
            Serial.print(PGM_READ_IDFLOAT(&me->weights[me->i_j]) MULTIPLY_BY_INT_IF_QUANTIZATION, DFLOAT_LEN);
            me->i_j++;
#else
            if (PGM_READ_IDFLOAT(&weights[i][j]) MULTIPLY_BY_INT_IF_QUANTIZATION > 0)
                Serial.print(F_MACRO(" "));
